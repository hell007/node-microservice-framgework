...
class SequelizeDbAdapter {
  ...
  /**
   * Query all entities by sql.
   * @param {*} filters 
   * @returns 
   */
  query(filters) {
    if(!_.isString(filters.sql) || _.isEmpty(filters.sql)) {
      throw new ServiceSchemaError('Missing sql definition in this query!')
    }
    ...
    let opt = {
      model: this.model,
      mapToModel: filters.mapToModel || true
    } 

    if(!_.isEmpty(filters.replacements)) {
      opt.replacements = filters.replacements
    }

    if(!_.isEmpty(filters.type)) {
      opt.type = filters.type
    }

    return this.db.query(sql, opt)
  }
  ...
  createCursor(params, isCounting) {
    ...
    // Select fields 2021-12-07 edit by myself
    if(Array.isArray(params.fields) && !_.isEmpty(params.fields)) {
      q.attributes = params.fields
    }
